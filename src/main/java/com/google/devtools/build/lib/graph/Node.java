...
final class Node<T> {
  ...
  Collection<Node<T>> getSuccessors() {
  ...
  /** Returns an (unordered, possibly immutable) set of the nodes that link to this node. */
  Collection<Node<T>> getPredecessors() {
  ...
  /**
   * Removes edge from this node to target.
   *
   * <p>In this method one lock is held inside another lock, but it cannot be the reason for a
   * reordering deadlock. The lock always holds in direction {@code fromNode.succs -> toNode.preds}.
   *
   * @see #addEdge(Node)
   * @return {@code true} if edge was removed, {@code false} otherwise
   */
